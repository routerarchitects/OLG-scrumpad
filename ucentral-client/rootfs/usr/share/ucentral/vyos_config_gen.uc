...
function tuple_to_ip(t) {
	return sprintf("%d.%d.%d.%d", t[0], t[1], t[2], t[3]);
}
...
function network_base(cidr) {
	let parts = split_cidr(cidr);
	if (!parts)
		return null;

	let ipt = ip_to_tuple(parts[0]);
	if (!ipt)
		return null;

	let p    = int(parts[1]);
	let mask = prefix_to_mask(p);
	let ipn  = tuple_to_int(ipt);
	let netn = ipn & mask;
	let nett = int_to_tuple(netn);

	return [
		tuple_to_ip(nett) + "/" + p, // 0: net_cidr
		parts[1],                    // 1: prefix string
		tuple_to_ip(nett),           // 2: net_ip
		netn,                        // 3: net_int
		p                            // 4: prefix int
	];
}
...
function add_host(ip, add) {
	let t = ip_to_tuple(ip);
	if (!t)
		return null;

	let n = tuple_to_int(t);
	n = (n + add) & 0xFFFFFFFF;
