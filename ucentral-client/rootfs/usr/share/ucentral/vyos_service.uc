	...
	if (type(config.interfaces) == "array") {
		for (let iface in config.interfaces) {
			...
			if (type(iface.ipv4) == "object" &&
				...
				let parts = split_cidr(iface.ipv4.subnet);
				if (parts) {
					let lan_ip = parts[0];

					let nb = network_base(iface.ipv4.subnet);
					if (nb) {
						let net_cidr = nb[0]; // 192.168.50.0/24
						...
						if (lt)
							lease_secs = convert_hours_to_seconds(lt, lease_secs);
			...
			if (type(iface.vif) == "array") {
				for (let v in iface.vif) {
					...
					let parts_v = split_cidr(v.subnet);
					if (!parts_v)
						continue;
					...
					let net_cidr_v = nbv[0]; // 192.168.10.0/24
					...
					if (lt_v)
						lease_secs_v = convert_hours_to_seconds(lt_v, lease_secs_v);
